...
# Run gams
  ...
  # For SNPs as continuous
   #   fixed_effects_int <- paste0("rgr ~ section_block + ", snp, " + s(height_2014, bs =\"cr\") + s(", climate_var_dif, " , bs = \"cr\") + s(", climate_var_dif,", by = ", snp, ", bs=\"cr\") + s(PC1_clim, bs =\"cr\") + s(PC2_clim, bs =\"cr\") + s(tmax_dif, by = PC1_clim, bs =\"cr\") + s(tmax_dif, by = PC2_clim, bs =\"cr\")")
   # 
 ...
 ## Calculate decrease in deviance with SNP data  
    ...
    #   form_no_snp <- formula(paste0(paste0("rgr ~ section_block + s(height_2014, bs=\"cr\") + s(", climate_var_dif,", bs=\"cr\") + s(locality, bs = \"re\") + s(accession, bs = \"re\") + s(tmax_dif, by = PC1_clim, bs = \"cr\") + s(tmax_dif, by = PC2_clim, bs = \"cr\") + s(PC1_gen, bs=\"cr\") + s(PC2_gen, bs=\"cr\") + s(", climate_var_dif,", by = PC1_gen, bs=\"cr\") + s(", climate_var_dif,", by = PC2_gen, bs=\"cr\")")))
    # 
